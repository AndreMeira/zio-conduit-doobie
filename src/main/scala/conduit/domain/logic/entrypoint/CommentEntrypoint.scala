...
trait CommentEntrypoint {
  ...
  def listComments(request: ListCommentsRequest): Result[CommentListResponse]

  def run(request: CommentRequest): Result[CommentResponse] = request match
